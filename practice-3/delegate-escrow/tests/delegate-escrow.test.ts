...
import { DelegateEscrow } from '../target/types/delegate_escrow';
import {
  Connection,
  Keypair,
  LAMPORTS_PER_SOL,
...
describe('escrow', () => {
  ...
  const program = anchor.workspace.delegateEscrow as Program<DelegateEscrow>;
  ...
  test('Offer is not taken by Bob, because Alice has spent the tokens', async () => {
    const offeredUsdc = new BN(10_000_000);
    const wantedWif = new BN(100_000_000);
    ...
    expect(takeOfferTx(offerAddress, bob)).rejects.toThrow();
});
});
