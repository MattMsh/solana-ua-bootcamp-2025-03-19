...
describe('escrow', () => {
  ...
  test('Close offer, tokens are returned to maker', async () => {
    const getTokenBalance = getTokenBalanceOn(connection);
    ...
    // Verify state before the offer is closed.
    const [aliceUsdcBalance, aliceWifBalance] = await Promise.all([
      getTokenBalance(aliceUsdcAccount),
      getTokenBalance(aliceWifAccount),
    ]);

    await closeOfferTx(alice, offerId);

    expect(program.account.offer.fetch(offerAddress)).rejects.toThrow();

    const [aliceUsdcBalanceAfter, aliceWifBalanceAfter] = await Promise.all([
      getTokenBalance(aliceUsdcAccount),
      getTokenBalance(aliceWifAccount),
    ]);

    expect(aliceUsdcBalanceAfter).toEqual(aliceUsdcBalance.add(offeredUsdc));
    expect(aliceWifBalanceAfter).toEqual(aliceWifBalance);
  });
});
